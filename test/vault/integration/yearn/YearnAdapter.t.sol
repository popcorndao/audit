...
contract YearnAdapterTest is AbstractAdapterTest {
    ...
    function test__malicious_initial_deposit() public {
        /*  If a malicicous user deposits a tiny amount of after adapter creation...
            ...
            From the Readme: The `totalAsset` issue is one that i think all ERC4626 implementations are suffering from. 
            A similiar issue that affects yearn is already known. 
            See Finding 3, "Division rounding may affect issuance of shares" for more details:
            https://github.com/yearn/yearn-security/blob/master/audits/20210719_ToB_yearn_vaultsv2/ToB_-_Yearn_Vault_v_2_Smart_Contracts_Audit_Report.pdf
            Yearn vaults are vulnerable to an expensive but possible attack where a malicious initial deposit can force share calculations to round down.
        */
        ...
        assertApproxEqAbs(
            adapter.balanceOf(address(alice)),
            _delta_,
            aliceDeposit
        );
        ...
        assertGt(
            adapter.convertToAssets(adapter.balanceOf(address(alice))),
            aliceDeposit + aliceAttack
        );
        emit log_uint(
            adapter.convertToAssets(adapter.balanceOf(address(alice)))
        );
