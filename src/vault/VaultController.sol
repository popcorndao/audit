...
contract VaultController is Owned {
    ...
    function _handleVaultStakingRewards(address vault, bytes memory rewardsData)
        internal
    {
    ...
    function __deployAdapter(
        ...
        adminProxy.execute(
            adapter,
            abi.encodeWithSelector(
                IAdapter.setManagementFee.selector,
                managementFee
            )
        );
    }
    ...
    function _registerVault(address vault, VaultMetadata memory metadata)
        internal
    {
        (bool success, bytes memory returnData) = adminProxy.execute(
     ...
     */
    function addTemplateCategories(bytes32[] calldata templateCategories)
        external
        onlyOwner
    {
    ...
    function _verifyCreatorOrOwner(address vault)
        internal
        returns (VaultMetadata memory metadata)
    {
    ...
    function _verifyCreator(address vault)
        internal
        view
        returns (VaultMetadata memory metadata)
    {
    ...
    /// @notice Verify that the token is valid.
    /// @dev Token allowlists can be toggled between an endorsementList to a rejectionList.
    /// @dev Per default assets need to get specifically rejected.
    ...
    function _verifyAdapterConfiguration(address adapter, bytes32 adapterId)
        internal
        view
    {
        if (adapter != address(0)) {
            if (adapterId > 0) revert AdapterConfigFaulty();
            if (!cloneRegistry.cloneExists(adapter))
                revert AdapterConfigFaulty();
        }
    }

    /// @notice Verify that the array lengths are equal.
    function _verifyEqualArrayLength(uint256 length1, uint256 length2)
        internal
        pure
    {
    ...
    /// @notice Access list to create Vaults,Adapters or Strategies
    /// @dev Access can be toggled between an endorsementList to a rejectionList
    /// @dev Per default addresses need to get specifically rejected.
    modifier canCreate() {
        if (
     ...
     */
    function setAdapterManagementFees(address[] calldata adapters)
        external
        onlyOwner
    {
     ...
     */
    function setAdapterHarvestCooldowns(address[] calldata adapters)
        external
        onlyOwner
    {
        uint8 len = uint8(adapters.length);
     ...
     */
    function setActiveTemplateId(bytes32 templateCategory, bytes32 templateId)
        external
        onlyOwner
    {
