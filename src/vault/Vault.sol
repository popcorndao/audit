    ...
    OwnedUpgradeable
{
        ...
        returns (uint256 shares)
    {
        ...
        uint256 feeShares = convertToShares(
            assets.mulDiv(fees.deposit, 1e18, Math.Rounding.Down)
        );
        ...
        returns (uint256 shares)
    {
        shares = adapter.previewDeposit(
            assets - assets.mulDiv(fees.deposit, 1e18, Math.Rounding.Down)
        );
    }
    ...
    function accruedManagementFee() public view returns (uint256) {
        ...
        return
            (fees.management.mulDiv(area, 2, Math.Rounding.Down) /
    ...
    function accruedPerformanceFee() public view returns (uint256) {
        ...
        if (shareValue > vaultShareHWM) {
            return
                fees.performance.mulDiv(
                    (shareValue - vaultShareHWM) * totalSupply(),
                    1e18 * ONE,
                    Math.Rounding.Down
                );
        } else {
