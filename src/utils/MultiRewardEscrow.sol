...
contract MultiRewardEscrow is Owned {
    ...
    mapping(address => mapping(IERC20 => bytes32[]))
        public userEscrowIdsByToken;
    ...
    function setFees(IERC20[] memory tokens, uint256[] memory tokenFees)
        ...
        onlyOwner
    {
        if (tokens.length != tokenFees.length)
            revert ArraysNotMatching(tokens.length, tokenFees.length);
