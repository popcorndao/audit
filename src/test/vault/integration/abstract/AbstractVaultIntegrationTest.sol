...
contract AbstractVaultIntegrationTest is Test {
    ...
    function test__deposit_withdrawl_pps_stays_constant(uint80 fuzzAmount)
        public
    {
        ...
        uint256 pps1;
        uint256 pps2;
        uint256 len = Math.min(
            uint256(testConfigStorage.getTestConfigLength()),
            maxConfigs
        );
        // solhint-disable
        for (uint256 i; i < len; i++) {
            ...
            for (uint256 i; i < 3; ++i) {
                ...
                assertWithin(
                    ...
                    pps2,
                    1,
                    string.concat(Strings.toString(i), "-deposit-", testId)
                );
            }
            // solhint-disable-next-line
            for (uint256 i; i < 2; ++i) {
                ...
                assertWithin(
                    ...
                    pps2,
                    2,
                    string.concat(Strings.toString(i), "-withdraw-", testId)
                );
            }
        }
    }

    function test__deposit_withdrawl_preview_resembles_actual(uint80 fuzzAmount)
        public
    {
        uint256 amount = bound(uint256(fuzzAmount), defaultAmount, maxDeposit);
        ...
        for (uint256 i; i < len; i++) {
            ...
            for (uint256 i; i < 3; ++i) {
                ...
                assertWithin(
                    ...
                    pps2,
                    1,
                    string.concat(Strings.toString(i), "-mint-", testId)
                );
            }
            // solhint-disable-next-line
            for (uint256 i; i < 2; ++i) {
                ...
                assertWithin(
                    ...
                    pps2,
                    1,
                    string.concat(Strings.toString(i), "-redeem-", testId)
                );
            }
        }
    }

    function test__mint_redeem_preview_resembles_actual(uint80 fuzzAmount)
        public
    {
        uint256 amount = bound(uint256(fuzzAmount), defaultAmount, maxDeposit);
