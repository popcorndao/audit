...
import {YearnAdapter, SafeERC20, IERC20, IERC20Metadata, Math, VaultAPI, IYearnRegistry, IAdapter} from "../../../../vault/adapter/yearn/YearnAdapter.sol";
...
contract YearnVaultTest is AbstractVaultIntegrationTest {
    ...
    function _setUpTest(bytes memory testConfig) internal {
        ...
        yearnVault = VaultAPI(
            IYearnRegistry(yearnRegistry).latestVault(_asset)
        );

        adapter.initialize(
            abi.encode(_asset, address(this), address(0), 0, sigs, ""),
            yearnRegistry,
            ""
        );
