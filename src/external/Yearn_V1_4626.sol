 ...
 * @author 0xTinder
 */
abstract contract YearnV1_4626 is
    ERC20,
    IVaultWrapper,
    IERC4626,
    ReentrancyGuard
{
    ...
    function calculateAssetsPlusFee(uint256 assets)
        internal
        view
        virtual
        returns (uint256 adjustedAmount);

    function calculateAssetsLessFee(uint256 assets)
        internal
        view
        virtual
        returns (uint256 adjustedAmount);
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        return
            convertYearnSharesToAssets(_vault.balanceOf(address(this)), _vault);
    }
    ...
    {
        IVaultV1 _vault = getVault();
        uint256 adjustedAmount = calculateAssetsPlusFee(assets);
    ...
    {
        IVaultV1 _vault = getVault();
        uint256 ySupply = _vault.balanceOf(address(this));
        uint256 yearnShares = ySupply == 0
            ? shares
            : shares.mulDivDown(ySupply, totalSupply());
        yearnShares = Math.min(_vault.balanceOf(address(this)), yearnShares);
        assets = calculateAssetsLessFee(
            convertYearnSharesToAssets(yearnShares, _vault)
        );
    }
  ...
  //////////////////////////////////////////////////////////////*/
        ...
        returns (uint256 assets)
    {
        ...
        require(
            shares == expectedMint,
            "Amount of shares minted does not match requested amount"
        );
    ...
    function redeem(
        ...
        require(
            _burntShares == shares,
            "must burn exactly the same number of shares"
        );
    ...
    function _deposit(
        ...
        uint256 _allowance = _token.allowance(address(this), address(_vault));
    ...
    function _withdraw(
        ...
        uint256 adjustedAmount = calculateAssetsPlusFee(amount);
        shares = _previewWithdraw(adjustedAmount, _vault);
        uint256 yearnShares = convertAssetsToYearnShares(
            adjustedAmount,
            _vault
        );
        ...
        uint256 dust = assets + beforeTokenBal - amount;
        if (dust > 0) {
    ...
    function _redeem(
        ...
        IERC20 _token = getToken();
        IVaultV1 _vault = getVault();
        uint256 yearnShares = convertSharesToYearnShares(shares, _vault);
    ...
    function _doWithdrawal(
        ...
        if (
            shares > balanceOf(sender) ||
            yearnShares > _vault.balanceOf(address(this))
        ) {
            revert NotEnoughAvailableSharesForAmount();
        }
    ...
    function convertAssetsToYearnShares(uint256 assets, IVaultV1 _vault)
        internal
        view
        returns (uint256 yShares)
    {
        uint256 totalYearnShares = _vault.totalSupply(); // Saves an extra SLOAD if totalSupply is non-zero.
        uint256 totalYearnAssets = _vault.balance();
        return
            totalYearnAssets == 0
                ? assets
                : assets.mulDivUp(totalYearnShares, totalYearnAssets);
    }

    function convertYearnSharesToAssets(uint256 yearnShares, IVaultV1 _vault)
        internal
        view
        returns (uint256 assets)
    {
        ...
        return
            supply == 0
                ? yearnShares
                : (yearnShares * _vault.balance()) / supply;
    }

    function convertSharesToYearnShares(uint256 shares, IVaultV1 _vault)
        internal
        view
        returns (uint256 yShares)
    {
    ...
    function _previewWithdraw(uint256 adjustedAmount, IVaultV1 _vault)
        internal
        view
        returns (uint256 shares)
    {
        ...
        return
            localAssets == 0
                ? adjustedAmount
                : adjustedAmount.mulDivUp(supply, localAssets);
    }
